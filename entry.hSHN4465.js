...
function clean_terms(input) {
  return (input || "").toLocaleLowerCase().split(" ").map((v) => v.trim()).flatMap((v) => [v, _.deburr(v)]).filter((value, index, array) => array.indexOf(value) === index);
}
...
function filterBooks(input2) {
  let yearToCount = {};
  if (input2.length === 0) {
    searchTermsToElement.forEach(([_2, element]) => {
  ...
  searchTermsToElement.forEach(([terms, element]) => {
    ...
    if (terms.some((term) => term.toLowerCase().includes(input2))) {
      element.removeAttribute("hidden");
      if (category === "past") {
        yearToCount[year] += 1;
      }
  ...
  _.chain(yearToCount).toPairs().filter(([year, count]) => year != "").sortBy((pair) => pair[0]).reverse().forEach(([year, count]) => {
...
readingInput.addEventListener("input", (event) => {
  let input2 = event.target.value.toLowerCase();
  filterBooks(input2);
});
