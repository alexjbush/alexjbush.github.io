(() => {
  ...
  function clean_terms(input2) {
    return import_lodash.default.chain(input2 || "").thru((v) => v.toLocaleLowerCase()).trim().split(" ").map((v) => v.trim()).flatMap((v) => [v, import_lodash.default.deburr(v)]).uniq().value();
  }
  ...
  function search_books() {
    ...
    ids_to_words.forEach((v) => {
      let m = import_lodash.default.every(terms, (t) => import_lodash.default.some(v.words, (w) => w.includes(t)));
      if (m) {
